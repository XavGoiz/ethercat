...
def makeTemplate(ai, longin, longout, bi, bo, output, base, devtype, revision, extraPdos):
    ...
    print(macro_header % { "base":        base , 
                                 "devtype":     devtype, 
                                 "revision":    hex(revision),
                                 "extrapdos":   pdolist}, file=f)
    for l in ["AL_STATE"]:
        print(al_text % {"name": fixname(l), "command": l}, file=f)
    for l in ["ERROR_FLAG"]:
        print(longin_text % {"name": fixname(l), "command": l}, file=f)
    for l in longin:
        print(longin_text % {"name": fixname(l), "command": l}, file=f)
    for l in bi:
        print(bi_text % {"name": fixname(l), "command": l}, file=f)
    for l in longout:
        print(longout_text % {"name": fixname(l), "command": l}, file=f)
    for l in bo:
        print(bo_text % {"name": fixname(l), "command": l}, file=f)
    for l in ai:
        print(ai_text % {"name": fixname(l), "command": l}, file=f)
    f.close()
