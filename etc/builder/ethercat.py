...
# 
# 'ethercat.py' defines these classes
# Sdo, SdoEntry, PdoEntry, PaddingEntry, Pdo
# SyncManager, EthercatDevice, EthercatChainElem, EthercatChain
#
# 'ethercat.py' defines these functions

# initialise():
# filteredDescriptions(dev_descriptions = None,filter = None):
# parseInt(text):
# parsePadding(entry):
# parsePdoEntry(entry, oversample):
# parsePdo(pdoNode, os, rxtx):
# parseSyncManager(smNode):
# getDescriptions(filename):
# getAllDevices():
# getPdoEntryChoices(all_devices):
# parseTypeRev(type_rev):

#
# 'getAllDevices' loads a dictionary generated by
# build_iocbuilder_descriptions.py and
# build_ethercat_descriptions.py 
#
...
def getAllDevices():
    '''load a dictionary of possible devices from the xml description files
