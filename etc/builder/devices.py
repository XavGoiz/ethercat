...
__all__ = ['EthercatMaster', 'EthercatSlave', 'GenericADC',
           'GenericADCTemplate','MasterTemplate', 'RtutilsLib',
           'ScannerLib']
...
# scanner start-up, used in EthercatMaster.writeScannerStartup
...
class EthercatDevice(Device):
    '''An EtherCAT device ancestor class to avoid duplicate inclusion
...
class EthercatMaster(Device):
    ...
    def Initialise(self):
        print('ecAsynInit("%(socket)s", %(max_message)d)'
               % self.__dict__)
...
class EthercatSlave(Device):
    ...
    def __init__(self, master, position, name, type_rev, oversample = 0):
        self.__super.__init__()
        self.master = master
        self.chainelem = ethercat.EthercatChainElem(
                                type_rev, position, name, oversample)
        self.name = name
    ...
    ArgInfo = makeArgInfo(
        __init__,
        master = Ident("master device for slave", EthercatMaster),
        position = Simple("slave position in ethercat chain, "
                          "or serial number of format DCS00001234",
                          str),
        type_rev = Choice("Device type and revision",ethercat.types_choice),
        name = Simple("slave's asyn port name", str),
        oversample = Simple("slave's oversampling rate, e.g. on "
